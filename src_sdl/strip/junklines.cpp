...
// Include framework headers FIRST so TetrimoneBoard is declared
#ifdef GTK3
...
// ============================================================================
// Core Junk Lines Generation (Framework-Independent)
// ============================================================================

// Helper function to fill a row with junk blocks
void fillJunkRow(std::vector<std::vector<int>>& grid, int rowIndex, int gridWidth) {
    // Create a list of empty spaces (at least 4 per row)
    int emptySpaces = 4 + std::rand() % (gridWidth / 3); // Between 4 and 1/3 of width
    std::vector<int> emptyPositions;
    
    // Generate positions for empty spaces
    while ((int)emptyPositions.size() < emptySpaces) {
        int pos = std::rand() % gridWidth;
...
void TetrimoneBoard::generateJunkLines(int percentage) {
    ...
    int junkLines = (gridHeight * percentage) / 100;
    
    // Ensure we don't fill more than the grid height
    junkLines = std::min(junkLines, gridHeight - 0);
    
    if (junkLines <= 0) return;
    
    // Fill bottom rows with junk
    for (int y = gridHeight - 1; y >= gridHeight - junkLines; y--) {
        fillJunkRow(grid, y, gridWidth);
    }
    ...
    if (currentPiece) {
        ...
        if (checkCollision(*currentPiece)) {
            ...
            for (int testX = 0; testX <= gridWidth - 4; testX++) {
            ...
            if (!foundValidPosition) {
                currentPiece->setPosition(gridWidth / 2 - 2, 0);
            }
        }
    }
}
...
void TetrimoneBoard::addJunkLinesFromBottom(int numLines) {
    ...
    numLines = std::min(numLines, gridHeight - 5); // Leave at least 5 rows at top
    ...
    for (int y = gridHeight - numLines; y < gridHeight; y++) {
        fillJunkRow(grid, y, gridWidth);
    }
...
// ============================================================================
// Framework-Specific Game Setup Dialog
// ============================================================================
...
// Callback for game setup dialog (GTK3)
...
void onGameSetupDialog(void* menuItem, void* userData) {
    TetrimoneApp* app = static_cast<TetrimoneApp*>(userData);
    
    // TODO: Implement Qt5 game setup dialog
}
