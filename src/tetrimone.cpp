...
std::array<double, 3> TetrimoneBlock::getColor() const {
    // Get the current level (need to add a way to pass this info to the TetrimoneBlock)
...
void createMenu(TetrimoneApp* app) {
    ...
    GtkWidget* boardSizeMenuItem = gtk_menu_item_new_with_label("Board Size...");
    gtk_menu_shell_append(GTK_MENU_SHELL(rulesMenu), boardSizeMenuItem);
    g_signal_connect(G_OBJECT(boardSizeMenuItem), "activate",
                   G_CALLBACK(onBoardSizeDialog), app);
    
    // Add a separator before the Minimum Block Size submenu
    gtk_menu_shell_append(GTK_MENU_SHELL(rulesMenu), gtk_separator_menu_item_new());
...
void TetrimoneBoard::setGridSize(int width, int height) {
    // Apply constraints to ensure the grid stays within sane boundaries
    width = std::max(MIN_GRID_WIDTH, std::min(width, MAX_GRID_WIDTH));
    height = std::max(MIN_GRID_HEIGHT, std::min(height, MAX_GRID_HEIGHT));
...
void onBoardSizeDialog(GtkMenuItem* menuItem, gpointer userData) {
    ...
    // Store original grid size in case user cancels
    int originalWidth = app->board->getGridWidth();
    int originalHeight = app->board->getGridHeight();
    ...
    // Add a heading label
    GtkWidget* headingLabel = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(headingLabel), "<b>Adjust Board Dimensions</b>");
    gtk_widget_set_halign(headingLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), headingLabel, FALSE, FALSE, 0);
    
    // Width adjustment section
    GtkWidget* widthLabel = gtk_label_new("Board Width:");
    gtk_widget_set_halign(widthLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), widthLabel, FALSE, FALSE, 0);
    
    // Create a horizontal scale for width
    GtkWidget* widthScale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 
                                                  MIN_GRID_WIDTH, MAX_GRID_WIDTH, 1);
    gtk_range_set_value(GTK_RANGE(widthScale), originalWidth);
    gtk_scale_set_digits(GTK_SCALE(widthScale), 0); // No decimal places
    gtk_scale_set_value_pos(GTK_SCALE(widthScale), GTK_POS_RIGHT);
    gtk_box_pack_start(GTK_BOX(vbox), widthScale, FALSE, FALSE, 0);
    
    // Add min/max labels for width
    GtkWidget* widthRangeBox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_box_pack_start(GTK_BOX(vbox), widthRangeBox, FALSE, FALSE, 0);
    
    GtkWidget* widthMinLabel = gtk_label_new("Narrow");
    gtk_widget_set_halign(widthMinLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(widthRangeBox), widthMinLabel, TRUE, TRUE, 0);
    
    GtkWidget* widthMaxLabel = gtk_label_new("Wide");
    gtk_widget_set_halign(widthMaxLabel, GTK_ALIGN_END);
    gtk_box_pack_end(GTK_BOX(widthRangeBox), widthMaxLabel, TRUE, TRUE, 0);
    
    // Current width value label
    GtkWidget* currentWidthLabel = gtk_label_new(NULL);
    char widthBuf[32];
    snprintf(widthBuf, sizeof(widthBuf), "Current width: %d", originalWidth);
    gtk_label_set_text(GTK_LABEL(currentWidthLabel), widthBuf);
    gtk_widget_set_halign(currentWidthLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), currentWidthLabel, FALSE, FALSE, 0);
    ...
    GtkWidget* heightLabel = gtk_label_new("Board Height:");
    gtk_widget_set_halign(heightLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), heightLabel, FALSE, FALSE, 0);
    
    // Create a horizontal scale for height
    GtkWidget* heightScale = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 
                                                   MIN_GRID_HEIGHT, MAX_GRID_HEIGHT, 1);
    gtk_range_set_value(GTK_RANGE(heightScale), originalHeight);
    gtk_scale_set_digits(GTK_SCALE(heightScale), 0); // No decimal places
    gtk_scale_set_value_pos(GTK_SCALE(heightScale), GTK_POS_RIGHT);
    gtk_box_pack_start(GTK_BOX(vbox), heightScale, FALSE, FALSE, 0);
    
    // Add min/max labels for height
    GtkWidget* heightRangeBox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_box_pack_start(GTK_BOX(vbox), heightRangeBox, FALSE, FALSE, 0);
    
    GtkWidget* heightMinLabel = gtk_label_new("Short");
    gtk_widget_set_halign(heightMinLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(heightRangeBox), heightMinLabel, TRUE, TRUE, 0);
    
    GtkWidget* heightMaxLabel = gtk_label_new("Tall");
    gtk_widget_set_halign(heightMaxLabel, GTK_ALIGN_END);
    gtk_box_pack_end(GTK_BOX(heightRangeBox), heightMaxLabel, TRUE, TRUE, 0);
    
    // Current height value label
    GtkWidget* currentHeightLabel = gtk_label_new(NULL);
    char heightBuf[32];
    snprintf(heightBuf, sizeof(heightBuf), "Current height: %d", originalHeight);
    gtk_label_set_text(GTK_LABEL(currentHeightLabel), heightBuf);
    gtk_widget_set_halign(currentHeightLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), currentHeightLabel, FALSE, FALSE, 0);
    
    // Warning about game restart
    GtkWidget* warningLabel = gtk_label_new(NULL);
    gtk_label_set_markup(GTK_LABEL(warningLabel), 
                       "<span foreground='red'>Note:</span> Changing board size will restart the game.");
    gtk_widget_set_halign(warningLabel, GTK_ALIGN_START);
    gtk_box_pack_start(GTK_BOX(vbox), warningLabel, FALSE, FALSE, 10);
    
    // Create callback data
    BoardSizeCallbackData cbData;
    cbData.app = app;
    cbData.widthLabel = currentWidthLabel;
    cbData.heightLabel = currentHeightLabel;
    cbData.widthScale = widthScale;
    cbData.heightScale = heightScale;
    
    // Connect signals for value changes
    g_signal_connect(G_OBJECT(widthScale), "value-changed", 
                   G_CALLBACK(updateBoardSizeValueLabels), &cbData);
    g_signal_connect(G_OBJECT(heightScale), "value-changed", 
                   G_CALLBACK(updateBoardSizeValueLabels), &cbData);
    ...
    if (response == GTK_RESPONSE_APPLY) {
        ...
        int newWidth = (int)gtk_range_get_value(GTK_RANGE(widthScale));
        int newHeight = (int)gtk_range_get_value(GTK_RANGE(heightScale));
        
        // Check if dimensions actually changed
        if (newWidth != originalWidth || newHeight != originalHeight) {
            // Apply the new dimensions and restart the game
            app->board->setGridSize(newWidth, newHeight);
            app->board->restart();
            resetUI(app);
            
            // Rebuild the UI with the new dimensions
            rebuildGameUI(app);
...
void updateBoardSizeValueLabels(GtkRange* range, gpointer data) {
    BoardSizeCallbackData* cbData = static_cast<BoardSizeCallbackData*>(data);
    TetrimoneApp* app = cbData->app;
    
    // Get the current values from both sliders
    int width = (int)gtk_range_get_value(GTK_RANGE(cbData->widthScale));
    int height = (int)gtk_range_get_value(GTK_RANGE(cbData->heightScale));
    
    // Update width label
    char widthBuf[32];
    snprintf(widthBuf, sizeof(widthBuf), "Current width: %d", width);
    gtk_label_set_text(GTK_LABEL(cbData->widthLabel), widthBuf);
    
    // Update height label
    char heightBuf[32];
    snprintf(heightBuf, sizeof(heightBuf), "Current height: %d", height);
    gtk_label_set_text(GTK_LABEL(cbData->heightLabel), heightBuf);
}
