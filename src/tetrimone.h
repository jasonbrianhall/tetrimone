...
private:
  ...
  std::vector<std::unique_ptr<TetrimoneBlock>>
      nextPieces; // Vector of next pieces (3)
  int score;
  int level;
  int linesCleared;
  bool gameOver;
  ...
  static const double TRAIL_SPAWN_DELAY =
      80.0; // milliseconds between trail segments
...
public:
  ...
  std::array<double, 3> getInterpolatedColor(int blockType,
  ...
  const std::vector<std::unique_ptr<TetrimoneBlock>> &getNextPieces() const {
    return nextPieces;
  }
  ...
  cairo_surface_t
      *oldBackground;      // Store the previous background during transition
  ...
  const std::string &getCurrentPropagandaMessage() const {
    return currentPropagandaMessage;
  }
  ...
  void createFireworkBurst(double centerX, double centerY,
                           const std::array<double, 3> &baseColor,
                           int particleCount);
  bool isFireworksActive() const { return fireworksActive; }
  const std::vector<FireworkParticle> &getFireworkParticles() const {
  ...
  void setMaxTrailSegments(int segments) {
    maxTrailSegments = std::max(1, std::min(segments, 15));
  }
...
cairo_surface_t *cairo_image_surface_create_from_memory(const void *data,
                                                        size_t length);
...
std::array<double, 3>
getHeatModifiedColor(const std::array<double, 3> &baseColor, float heatLevel);
void drawFreezyEffect(cairo_t *cr, double x, double y, double size,
                      float heatLevel, double time);
void drawFireyGlow(cairo_t *cr, double x, double y, double size,
                   float heatLevel, double time);
