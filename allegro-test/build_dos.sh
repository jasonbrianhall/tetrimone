#!/bin/bash
# Combined build script for tetrimone DOS with Allegro 4
...
"dos")
    echo "Building tetrimone virtualizer for DOS..."
    
    # Ensure Allegro is built
    ./build_dos.sh allegro
    
    # Check source files
    if [ ! -f "tetrimone.cpp" ]; then
        echo "ERROR: tetrimone.cpp not found!"
        exit 1
    fi
    ...
    echo ""
    echo "🎮 tetrimone DOS Virtualizer built with Allegro 4!"
    echo "📁 Files:"
    rm $BUILD_DIR/tetrimone
...
"compile")
    ...
    docker run --rm \
        ...
        "
    rm $BUILD_DIR/tetrimone -f
    echo "🎮 tetrimone DOS Virtualizer compiled!"
    echo "📁 Files:"
    ls -la $BUILD_DIR/* 2>/dev/null || true
    ;;

"run")
    if [ ! -f "$BUILD_DIR/tetrimone.exe" ]; then
        echo "tetrimone virtualizer not built yet. Building..."
        ./build_dos.sh dos
    fi
    
    echo "Running tetrimone virtualizer in DOSBox..."
    cd $BUILD_DIR && dosbox tetrimone.exe
    ;;
